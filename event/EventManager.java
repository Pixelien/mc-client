...
public class EventManager {
   ...
   private static void sortListVaule(final Class<? extends Event> clazz) {
	   
	   final ArrayList<EventData> flexableArray = new ArrayList<EventData>();
	   
	   for(final byte b : EventPriority.VAULE_ARRAY) {
   ...
   private static boolean isMethodBad(final Method method) {
	   return method.getAnnotatedParameterTypes().length != 1 || !method.isAnnotationPresent(EventTarget.class);	
   }
...
public static void register(final Method method, final Object o) {
	...
	final EventData methodData = new EventData(o, method, method.getAnnotation(EventTarget.class).vaule());
	...
	if(REGISTRY_MAP.containsKey(clazz)) {
		...
		if(!REGISTRY_MAP.get(clazz).contains(methodData)) {
			...
			sortListVaule((Class<? extends Event>) clazz);
		}
